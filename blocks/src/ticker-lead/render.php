...
$stop_text = isset( $attributes['stopText'] ) && '' !== $attributes['stopText'] ? (string) $attributes['stopText'] : 'STOP';
$pause_ms  = isset( $attributes['pauseMs'] ) ? max( 500, (int) $attributes['pauseMs'] ) : 5000;
$size      = isset( $attributes['size'] ) ? (int) $attributes['size'] : 14;

// Seed the IxAPI store so the SSR pass for data-wp-text="state.label"
// resolves to the same string the client-side getter will compute once
// view.js hydrates. liveText/stopText are author-editable, pauseMs is
...
printf(
	'<div %1$s><a class="mh-site-mark" href="%2$s" aria-label="%3$s" title="%3$s">%4$s</a><button type="button" class="mh-ticker__live" data-wp-on--click="actions.togglePause" data-wp-text="state.label" aria-label="Pause ticker">%5$s</button></div>',
	$wrapper_attrs,
	$href,
	esc_attr( $label ),
	$svg,
